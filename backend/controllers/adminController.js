  ...
  try {
    ...
    const finalAppointments = await Promise.all(
      appointments.map(async (appt) => {
        ...
        return {
          ...appt,
          userData: user || { name: "Unknown", image: "", dob: null },
          docData: doctor || { name: "Unknown", image: "", speciality: "" }
        }
      })
    )

    res.json({ success: true, appointments: finalAppointments })
  } catch (error) {
    console.log(error)
    res.json({ success: false, message: error.message })
  }
...
const appointmentCancel = async (req, res) => {
  try {
    ...
    // update appointment
    const updateData = { cancelled: true }
...
const adminDashboard = async (req,res)=>{
  try{
    ...
    console.log("🔥 adminDashboard HIT");
    const dashData={
